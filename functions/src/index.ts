...
const getAnthropicApiKey = async (): Promise<string | null> => {
  try {
    ...
    console.warn('⚠️ Failed to retrieve Anthropic API key from Firestore:', error?.message);
  }
...
app.post('/api/extract-job-url', async (req: any, res: any) => {
  try {
    const { url } = req.body;
    ...
    try {
      ...
      const directResponse = await fetch(normalizedUrl, {
        ...
        signal: controller.signal as any, // Cast to any to avoid TS issues
        redirect: 'follow'
      });
          ...
          for (const match of jsonLdMatches) {
            try {
              ...
              if (Array.isArray(data)) {
                ...
                jobEntity = (data['@graph'] as any[]).find(checkEntity);
              }
    ...
    if (!scrapedContent) {
      try {
        ...
        if (jinaResponse.ok) {
          const jinaData = await jinaResponse.json() as any;
