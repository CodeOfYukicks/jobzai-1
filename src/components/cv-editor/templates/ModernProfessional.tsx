                      ...
                      {cvData.experiences.map(exp => (
                        <ClickableSection key={exp.id} sectionType="experience" itemId={exp.id} onSectionClick={onSectionClick} highlightTarget={highlightTarget}>
                          <div>
                            ...
                            {/* Support both 'bullets' and 'highlights' for backwards compatibility */}
                            {((exp.bullets || (exp as any).highlights || []).length > 0) && (
                              <ul className="list-disc list-inside space-y-1">
                                {(exp.bullets || (exp as any).highlights || []).map((bullet: string, idx: number) => (
