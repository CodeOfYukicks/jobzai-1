...
import RewritePreviewModal from '../assistant/RewritePreviewModal';
import RewriteAnimationWrapper from '../assistant/RewriteAnimationWrapper';
...
export default function NotesAIPopover({
  ...
  const handleAction = async (action: AIAction) => {
    try {
      // Start rewrite workflow - this will open the preview modal
      await startRewrite(selectedText, action);
      
      // Close the popover after starting the workflow
      onClose();
    } catch (error) {
      console.error('Error starting rewrite:', error);
      notify.error('Failed to start rewrite. Please try again.');
    }
  };
  
  const handleAcceptRewrite = async () => {
    await acceptRewrite();
    // Modal will close automatically after accepting
  };
  
  const handleRejectRewrite = () => {
    rejectRewrite();
    // Modal will close automatically after rejecting
  };
