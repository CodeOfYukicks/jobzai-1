...
interface AddInterviewFormProps {
  ...
  onSubmit?: (interview: Omit<Interview, 'id'>) => Promise<void>;
  onCancel: () => void;
  isCampaignMode?: boolean;
  hideHeader?: boolean;
}

export const AddInterviewForm = ({ onAdd, onSubmit, onCancel, isCampaignMode = false, hideHeader = false }: AddInterviewFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      // Support both onAdd and onSubmit props
      if (onSubmit) {
  ...
  return (
    <div className={hideHeader ? '' : 'mb-4'}>
      <div className={hideHeader ? '' : 'bg-white dark:bg-[#2b2a2c] border border-gray-200 dark:border-[#3d3c3e] rounded-2xl p-4 sm:p-5 shadow-xl shadow-gray-200/50 dark:shadow-none'}>
        {!hideHeader && (
