...
import { FileText, Check, Loader2, ExternalLink, Paperclip } from 'lucide-react';
...
// Source badge config
const SOURCE_CONFIG = {
  main: { label: 'Profile', color: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' },
  'resume-builder': { label: 'Resume Builder', color: 'bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20' },
  document: { label: 'PDF', color: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20' }
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          className={`
            ...
            ${(isLoadingCVs || cvOptions.length === 0) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
            focus:outline-none focus:ring-2 focus:ring-[#b7e219]/30 focus:ring-offset-2 dark:focus:ring-offset-[#1a1a1a]
          `}
        >
          <motion.div
            layout
            ...
            className={`
              ...
              ${attachEnabled
                ? 'bg-white ml-6'
                : 'bg-white dark:bg-gray-300 ml-0'
              }
            `}
          />
        </button>
      ...
      {!isLoadingCVs && cvOptions.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-[13px] text-gray-500 dark:text-white/60 max-w-xs mx-auto">
          ...
          {attachEnabled ? (
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={`
                        w-full group relative flex items-center gap-4 p-4 rounded-2xl text-left
                        transition-all duration-300 ease-out
                        ${isSelected
                          ? 'bg-[#b7e219]/10 dark:bg-[#b7e219]/[0.08] ring-2 ring-[#b7e219] shadow-[0_0_0_4px_rgba(183,226,25,0.1)]'
                          : 'bg-white/60 dark:bg-white/[0.03] hover:bg-white dark:hover:bg-white/[0.05] border border-gray-200/60 dark:border-white/[0.06] hover:border-gray-300 dark:hover:border-white/[0.1]'
                        }
                        backdrop-blur-sm
                      `}
                    >
                      ...
                      <div className={`
                        ...
                        ${isSelected
                          ? 'bg-[#b7e219]'
                          : 'border-2 border-gray-300 dark:border-gray-600 group-hover:border-[#b7e219]/50'
                        }
                      `}>
                        {isSelected && (
                          <motion.div
                      ...
                      <div className={`
                        ...
                        transition-all duration-300
                        ${isSelected
                          ? 'bg-[#b7e219]/20 dark:bg-[#b7e219]/25'
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <p className={`
                            text-[14px] font-medium truncate transition-colors duration-300
                            ${isSelected
            ...
            >
              <div className="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-white/[0.04] flex items-center justify-center mx-auto mb-4">
                <Paperclip className="w-6 h-6 text-gray-400 dark:text-gray-500" />
              </div>
              <p className="text-[14px] text-gray-500 dark:text-white/60">
                CV attachment disabled
              </p>
              <p className="text-[12px] text-gray-400 dark:text-gray-600 mt-1">
                Emails will be sent without resume attachment
              </p>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {!isLoadingCVs && attachEnabled && selectedCV && (
        <motion.div
          ...
          className="flex items-center gap-3 px-4 py-3 rounded-xl 
            bg-[#b7e219]/5 dark:bg-[#b7e219]/[0.06] 
            border border-[#b7e219]/20 dark:border-[#b7e219]/15"
        >
          <Check className="w-4 h-4 text-[#b7e219] flex-shrink-0" />
          <p className="text-[13px] text-gray-700 dark:text-gray-300">
            <span className="font-medium">{selectedCV.name}</span> will be attached to each email
          </p>
        </motion.div>
      )}
    </div>
