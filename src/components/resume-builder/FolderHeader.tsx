...
import { Image, Camera, X, Smile, Folder as FolderIcon } from 'lucide-react';
...
export default function FolderHeader({
  ...
  return (
    ...
    >
      {/* Cover Photo Area */}
      <div className={`relative w-full transition-all duration-300 ease-in-out ${hasCover ? 'h-48 sm:h-64' : 'h-24 sm:h-32'}`}>
        {hasCover ? (
          ...
          <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800/50 dark:to-gray-800 border-b border-gray-200 dark:border-gray-800">
      ...
      <div className="px-6 sm:px-8 pb-6 relative">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 -mt-8 sm:-mt-10 relative z-10">
          <div className="flex items-end gap-4">
            {/* Icon */}
            <div className={`relative shrink-0 flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-xl shadow-lg border-4 border-white dark:border-[#121212] ${folder ? 'bg-white dark:bg-gray-800' : 'bg-gray-100 dark:bg-gray-800'}`}>
              {folder && (folder.icon.length <= 2 || /^\p{Emoji}/u.test(folder.icon)) ? (
            ...
            <div className="mb-1.5 sm:mb-2">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 font-medium mt-0.5">
                <span>{subtitle}</span>
              </div>
            </div>
          </div>
