...
const TabButton = ({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2.5 text-[12px] sm:text-[13px] font-medium rounded-lg
        transition-all duration-200 whitespace-nowrap flex-shrink-0 min-h-[44px] ${active
    ...
    >
      <Icon className={`h-4 w-4 stroke-[1.5] flex-shrink-0 ${active ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'}`} />
      <span className="hidden xs:inline">{label}</span>
      {count !== undefined && count > 0 && (
        <span className={`px-1.5 py-0.5 rounded text-[10px] sm:text-[11px] font-medium tabular-nums
          ${active
        ...
        <div className="hidden md:block w-full px-3 sm:px-6 lg:px-8">
          ...
          <div className="pt-6 pb-4">
            ...
            <motion.div
              ...
              className="border-b border-border overflow-x-auto scrollbar-hide -mx-3 sm:mx-0 px-3 sm:px-0"
            >
